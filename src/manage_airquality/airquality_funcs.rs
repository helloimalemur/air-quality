...
pub async fn fetch_data_fire_alerts(
    ...
    let insert_url = format!("http://127.0.0.1:{}/api/addaq", settings_map.get("port").unwrap().clone());
    let _client = reqwest::Client::new().post(insert_url)
    ...
    if subs.is_ok() {
        for (_x,i) in subs.unwrap().iter().enumerate() {
            ...
            if new_airquality.data.current.pollution.aqius > i.max_aqi as i64 {
                println!("{} {} || {} > {}", new_airquality.data.city.clone(), i.email, i.max_aqi, new_airquality.data.current.pollution.aqius);
                fire_alert(i.email.to_string(), i.discord.to_string(), i.max_aqi.to_string(), new_airquality.data.current.pollution.aqius.to_string(), new_airquality.data.city.clone(), pool).await;
            }
        }
    }
}

pub async fn fire_alert(email: String, discord: String, max_aqi: String, current_aqi: String, current_city: String, _pool: &rocket::State<MySqlPool>) {
